...
def patch_model_and_tokenizer(
    ...
    for name, module in model.named_modules():
        if isinstance(module, (Bnb_Linear4bit, Peft_Linear4bit)):
            ...
            #figure out which state was not quantized bassically.....
...
def check_conversion_mappings(model, current_key_name_str, skip_modules):
    #breakpoint()
    if hasattr(model, "_root_cls") and hasattr(model._root_cls, "_checkpoint_conversion_mapping") and len(model._root_cls._checkpoint_conversion_mapping) > 0:
